...
export function SpeakerCard({
  ...
  const cats = subcategoryIds.slice(0, 2).map((id) => subcategoryMap[id]).filter(Boolean);
...
export function SpeakerRow({ speaker, subcategoryMap, subcategoryIds, index }: SpeakerRowProps) {
  const [hover, setHover] = useState(false);
  const cats = subcategoryIds.slice(0, 2).map((id) => subcategoryMap[id]).filter(Boolean);
  ...
  return (
    ...
    >
      ...
      <div>
        <h4
          className="serif"
          style={{ fontSize: 24, fontWeight: 400, lineHeight: 1.1, letterSpacing: "-0.02em" }}
        >
          {speaker.name}{" "}
          <span className="en" style={{ fontSize: 12, color: "var(--ink-muted)", marginLeft: 8, letterSpacing: "0.1em" }}>
            {speaker.name_en}
          </span>
        </h4>
